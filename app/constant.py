TAKE_LIST = ["Wallah c'est trop bien", "J'adore", "J'approuve", "C'est plutôt bien",
             "Je déteste", "Je désapprouve", "C'est éclaté", "C'est claqué au sol"]

ADVICE_LIST = ["Carrément ouais", "Vas-y fonce !", "Pourquoi pas ouais",
               "Je pense c'est une  mauvaise idée", "Absolument pas", "Tu devrais pas non"]

MAGIC_BALL_LIST = ["Essaye plus tard", "Essaye encore", "Pas d'avis", "C'est ton destin", "Le sort en est jeté",
                   "Une chance sur deux", "Repose ta question",
                   "D'après moi oui", "C'est certain", "Oui absolument", "Tu peux compter dessus", "Sans aucun doute",
                   "Très probable", "Oui", "C'est bien parti",
                   "C'est non", "Peu probable", "Faut pas rêver", "N'y compte pas", "Impossible"]

HELP_MESSAGE = "Les commandes disponibles actuellement sont : \n" \
               "- **CBB: Ton avis** où je donne mon avis sur les questions que vous me posez \n" \
               "- **CBB 8ball:** où je dis votre avenir à la manière d'une Magic 8 Ball \n" \
               "- **CBB horoscope:** il vous suffit de taper n'importe quelle phrase contenant le signe astrologique " \
               "et je vous donne votre horoscope \n" \
               "- **CBB: Est-ce que ** où je donne mon avis si vous devriez faire un truc ou non \n" \
               "- **CBB: Ping!** où je réponds Pong! \n" \
               "Respectez bien l'écriture des commandes comme affichées en gras !"

HOROSCOPE_LIST = ["Bélier", "Taureau", "Gémeaux", "Cancer", "Lion", "Vierge", "Balance", "Scorpion", "Sagittaire",
                  "Capricorne", "Verseau", "Poissons"]

HOROSCOPE_LINK = "https://www.evozen.fr/horoscope/horoscope-du-jour"

GOOD_NIGHT_LIST = ["bonne nuit", "je vais me coucher", "j'vais me coucher"]
